...
def read_intent(text: str, snapshot: PortfolioSnapshot) -> ReadIntentResult:
    ...
    if "buy" in lowered or "purchase" in lowered:
        ...
        intent = "holdings"
    observed = [f"{h.asset}: {h.quantity} units, ${h.value_usd:.2f} current value" for h in snapshot.holdings]
    calculated = []
    unknown = []
    ...
    if intent in ("last_purchase", "change_since_date"):
        return ReadIntentResult(intent=intent, observed=observed, calculated=[], assumed=[], unknown=unknown)
    if intent == "pnl":
    ...
    return ReadIntentResult(intent=intent, observed=observed, calculated=calculated,
