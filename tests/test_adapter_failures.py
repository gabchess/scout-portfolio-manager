...
def make_preview(destination="wallet:0xdef456"):
    return build_preview(
        intent=DcaIntent(asset="ETH", amount_usd=300, chain="ethereum", schedule="one_time",
                         source="wallet:0xabc123", destination=destination),
...
def test_fake_adapter_rejects_insufficient_balance_and_wrong_destination():
    with pytest.raises(ExecutionError, match="insufficient balance"):
        FakeExecutionAdapter(balance_usd=100).execute(make_preview(), approval=True, idempotency_key="funds")
...
def test_fake_adapter_can_expose_timeout_without_claiming_success():
    with pytest.raises(ExecutionError, match="timeout"):
        FakeExecutionAdapter(failure="timeout").execute(make_preview(), approval=True, idempotency_key="timeout")


def test_fake_adapter_rejects_unsupported_chain():
    with pytest.raises(ExecutionError, match="unsupported chain"):
        FakeExecutionAdapter(supported_chains={"base"}).execute(make_preview(), approval=True, idempotency_key="chain")
